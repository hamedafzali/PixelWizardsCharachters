...
export function poseToTransform(p) {
    const { x = 0, y = 0, rot = 0, rotY = 0, sx = 1, sy = 1 } = p;
    const parts = [];
    if (x || y)
        parts.push(`translate(${round(x)}px,${round(y)}px)`);
    if (rot)
        parts.push(`rotate(${round(rot)}deg)`);
    if (rotY)
        parts.push(`rotateY(${round(rotY)}deg)`);
    if (sx !== 1 || sy !== 1)
 ...
 */
export function composePoses(poses) {
    const out = { x: 0, y: 0, rot: 0, rotY: 0, sx: 1, sy: 1 };
...
function counter(v, ratio, cap, k) {
 ...
 */
function anticipationPose(peak, k) {
 ...
 */
export function buildAction(name, a) {
    ...
    stops.set(0, {});
    if (ant > 0)
        stops.set(antT, anticipationPose(peakOf(a.poses), ant));
    ...
    if (set > 0) {
        for (const [local, pose] of settleBeats(set))
            stops.set(actEnd + setT * local, pose);
    }
...
export function ensureAction(doc, name, a) {
