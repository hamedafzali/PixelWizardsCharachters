...
import { ensureAction, composePoses, poseToTransform } from './timing.js';
import { IdleMicroMotion, blinkStyle } from './micro.js';
import { FrameInterpolator } from './interpolate.js';
import { LocomotionDriver, LOCO_TARGETS, drivesGait } from './locomotion.js';
import { planGesture } from './gestures.js';
...
export class ActorRig {
    constructor(spec, opts = {}) {
        ...
        this.loco = new LocomotionDriver();
    ...
    mount(host) {
        ...
        this.render();
        if (this.opts.blink && !this.reduced)
            this.scheduleBlink();
        if (!this.reduced)
            this.startLoop();
        return this;
     ...
     */
    setContribution(target, source, pose) {
    ...
    targetEl(t) {
        if (t === 'root')
            return this.root;
        if (t === 'body')
            return this.svg?.querySelector('.rig-body') ?? null;
    ...
    flushTransforms() {
        for (const [target, sources] of this.contrib) {
            ...
            if (el)
                el.style.transform = poseToTransform(composePoses(sources.values()));
        }
    }
    ...
    render() {
        ...
        for (const n of ['shadow', 'accBack', 'tail', 'farArm', 'farLeg', 'torso',
            'nearLeg', 'nearArm', 'earL', 'earR', 'head', 'accFront']) {
            const el = this.svg.querySelector(`.lyr-${n}`);
            if (el)
                this.layers[n] = el;
        }
        this.injectLids();
    ...
    injectLids() {
        if (!this.svg)
            return;
        const parent = this.layers.head ?? this.svg.querySelector('.rig-flip') ?? this.svg;
    ...
    drivenGait() {
        ...
        const has = (t) => t === 'root' || t === 'torso' || !!this.layers[t];
    ...
    tickLocomotion(dt) {
        ...
        for (const t of LOCO_TARGETS) {
            if (t !== 'root' && t !== 'torso' && !this.layers[t])
                continue;
    ...
    playGesture(g) {
        if (!this.svg)
            return;
        const svg = this.svg;
        const plan = planGesture(g, this.frame.locomotion, this.drivenGait(), (layer) => !!svg.querySelector(`.${layer}`), this.spec.gestureFallback);
        ...
        for (const part of plan.parts) {
            const el = svg.querySelector(`.${part.layer}`);
            if (!el)
                continue;
        ...
        window.setTimeout(() => {
            for (const { el, animation } of started) {
                if (el.style.animation === animation)
                    el.style.animation = '';
            }
            this.clearGesture(g);
        }, dur * 1000);
     ...
     */
    clearGesture(g) {
        if (this.interp.goal.gesture === g)
            this.apply({ gesture: null });
    }
    /**
