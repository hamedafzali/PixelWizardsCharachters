...
export class IdleMicroMotion {
    ...
    tick(dt, frame) {
        ...
        if (a <= 0)
            return { torso: {}, head: {}, gaze: { x: 0, y: 0 } };
        // --- breath -----------------------------------------------------------
        const period = BREATH_PERIOD[frame.emotion] ?? 3.8;
        const b = Math.sin((this.t / period) * TAU);
        const torso = { sy: 1 + 0.014 * b * a, sx: 1 - 0.008 * b * a };
        const head = { y: -0.9 * b * a };
