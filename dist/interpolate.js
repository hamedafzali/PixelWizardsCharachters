...
const NUMERIC = ['intensity', 'mouthOpen', 'gazeX', 'gazeY', 'browRaise', 'speed'];
const DISCRETE = ['emotion', 'viseme', 'facing', 'locomotion', 'character', 'gesture'];
const DEFAULT_EPSILON = 0.001;
function clampChannel(c, v) {
...
export class FrameInterpolator {
    ...
    get settling() {
        for (const c of NUMERIC)
            if (this.num[c].elapsed !== Infinity)
                return true;
        for (const c of DISCRETE)
            if (this.dis[c].elapsed !== Infinity)
                return true;
        return false;
    }
...
function readPartialNumeric(f, c) {
    if (c === 'gazeX' || c === 'gazeY') {
        if (!f.gaze)
            return undefined;
