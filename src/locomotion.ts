...
export const LOCO_TARGETS: readonly LocoTarget[] = ['root', 'torso', 'farArm', 'nearArm', 'farLeg', 'nearLeg']
...
const IDENTITY: Pose = { x: 0, y: 0, rot: 0, sx: 1, sy: 1 }
 ...
 * that snaps at 1.0.
 */
export function samplePhase(keys: readonly PhaseKey[], phase: number): Pose {
  const first = keys[0]
  if (!first) return { ...IDENTITY }
  if (keys.length === 1) return { ...IDENTITY, ...first.pose }
 ...
 */
export function canDrive(gait: GaitName, has: (t: LocoTarget) => boolean): boolean {
 ...
 * growing a layer.
 */
export function drivesGait(
  gait: GaitName,
  has: (t: LocoTarget) => boolean,
  declared: readonly GaitName[] | undefined,
): boolean {
...
export class LocomotionDriver {
  ...
  pose(target: LocoTarget): Pose {
    if (this.gait === null) return { ...IDENTITY }
    const keys = GAITS[this.gait].tracks[target]
    if (!keys) return { ...IDENTITY }
